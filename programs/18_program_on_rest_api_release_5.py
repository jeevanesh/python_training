...
Supported DELETE
So, Finally
we have all GET/POST/PUT/PATCH/DELETE implemented here
"""
...
# -----------
# ENDPOINT -4 : URL http://127.0.0.1:5000/patchdbdata mapped to route("/patchdbdata")
# To Access Use: consume_api.py
#############
@my_rest_api_app.route("/patchdbdata", methods=["PATCH"])
def my_patchdbdata():
    # Get data sent by client
    new_record = flask.request.get_json()
    # new_record will be dictionary
...
# -----------
# ENDPOINT -5 : URL http://127.0.0.1:5000/deletedbdata mapped to route("/deletedbdata")
# To Access Use: consume_api.py
#############
@my_rest_api_app.route("/deletedbdata", methods=["DELETE"])
def my_deletedbdata():
    # Get data sent by client
    new_record = flask.request.get_json()
    # new_record will be dictionary
    ...
    records_count = my_db_result[0]
    if records_count == 0:
        response = flask.make_response(flask.jsonify("Record Not Present To Delete"), 404)
        return response
    else:
        my_query = f"""
        ...
        my_db_connection.commit()
        my_db_connection.close()
        response = flask.make_response(flask.jsonify("Record Deleted"), 200)
        return response
###############################
